...
> **Signing status.** macOS binaries in this release are **ad-hoc
> signed** by the release workflow (`codesign --sign -`). That is
> enough to satisfy the Apple Silicon kernel's "must be signed"
> requirement, so the binary launches without a SIGKILL — but it is
> *not* a Developer ID signature and is *not* notarized by Apple.
> Browser-downloaded binaries will still hit a Gatekeeper warning on
> first run; the `install.sh` curl-pipe path strips the quarantine
> attribute automatically. A future release will land Developer ID +
> notarization once the GitHub Actions secrets are configured (see
> [docs/RELEASE_SIGNING.md](docs/RELEASE_SIGNING.md)). The workflow
> is already wired for it — it activates the moment the secrets land.
